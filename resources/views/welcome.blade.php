...
<head>
    ...
    <style>
        :root { --navy:#111c35; --ink:#1d2942; --muted:#667189; --paper:#f7f8fc; --line:#e1e5ee; --blue:#4058dc; --violet:#7556df; --mint:#baf0d9; --coral:#ffb9a7; --yellow:#ffe189; }
        * { box-sizing:border-box; } html { scroll-behavior:smooth; } body { margin:0; color:var(--ink); background:var(--paper); font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif; } a { color:inherit; text-decoration:none; }
        ...
        .brand { display:inline-flex; gap:10px; align-items:center; font-size:19px; font-weight:850; letter-spacing:-.04em; } .mark { width:35px; height:35px; display:grid; place-items:center; color:#fff; border-radius:11px; background:linear-gradient(135deg,var(--blue),var(--violet)); box-shadow:0 10px 23px #4b59ca3d; } .mark svg { width:20px; }
        .navlinks { display:flex; gap:27px; color:var(--muted); font-size:14px; font-weight:700; } .navlinks a:hover,.signin:hover { color:var(--blue); } .nav-actions { display:flex; align-items:center; gap:12px; font-size:14px; font-weight:750; } .signin { color:var(--muted); padding:10px 6px; } .language { position:relative; } .language summary { cursor:pointer; list-style:none; padding:9px 7px; color:var(--muted); } .language summary::-webkit-details-marker { display:none; } .language summary:after { content:"v"; margin-left:5px; font-size:10px; } .language-menu { position:absolute; z-index:5; top:38px; right:0; width:178px; max-height:270px; overflow:auto; padding:7px; border:1px solid var(--line); border-radius:12px; background:#fff; box-shadow:0 16px 35px #1b294520; } .language-menu a { display:block; padding:8px 9px; border-radius:8px; color:var(--ink); font-size:13px; font-weight:650; } .language-menu a:hover { color:var(--blue); background:#eef1ff; }
        ...
        .button { display:inline-flex; align-items:center; justify-content:center; gap:9px; padding:12px 17px; border:1px solid transparent; border-radius:11px; font:inherit; font-weight:800; transition:transform .18s,box-shadow .18s,background .18s; } .button:hover { transform:translateY(-2px); } .primary { color:#fff; background:var(--navy); box-shadow:0 10px 22px #15213b29; } .primary:hover { background:var(--blue); box-shadow:0 14px 30px #4058dc40; } .secondary { background:#fff; color:var(--ink); border-color:var(--line); }
        .hero { position:relative; overflow:hidden; padding:90px 0 74px; background:radial-gradient(circle at 82% 20%,#e5e9ff 0,transparent 25%),radial-gradient(circle at 10% 0,#e3f8ed 0,transparent 25%),var(--paper); } .hero:before { content:""; position:absolute; inset:0; opacity:.45; background-image:radial-gradient(#aeb8d0 1px,transparent 1px); background-size:25px 25px; mask-image:linear-gradient(black,transparent 75%); }
        .hero-grid { position:relative; display:grid; grid-template-columns:.98fr 1.02fr; gap:56px; align-items:center; } .eyebrow { display:inline-flex; gap:8px; align-items:center; padding:7px 11px; border-radius:999px; color:#3b54aa; background:#e8edff; font-size:11px; font-weight:850; letter-spacing:.08em; text-transform:uppercase; } .eyebrow i { width:7px; height:7px; border-radius:50%; background:#50c88c; box-shadow:0 0 0 4px #50c88c2b; }
        h1 { max-width:600px; margin:19px 0; font-size:clamp(44px,5.4vw,69px); line-height:.99; letter-spacing:-.07em; } .accent { color:var(--blue); } .lede { max-width:540px; margin:0 0 28px; color:var(--muted); font-size:18px; line-height:1.65; } .actions { display:flex; flex-wrap:wrap; gap:12px; }.note { display:flex; align-items:center; gap:10px; margin-top:29px; color:var(--muted); font-size:13px; }.tick { display:grid; place-items:center; width:21px; height:21px; flex:0 0 auto; border-radius:50%; color:#126b4b; background:var(--mint); font-size:12px; font-weight:900; }
        ...
        .panel { display:grid; grid-template-columns:105px 1fr; gap:15px; padding-top:15px; } .side { display:grid; align-content:start; gap:10px; padding:7px 6px; }.side span { height:10px; border-radius:8px; background:#e9edf5; }.side span:nth-child(2){width:75%;background:#dfe4ff}.side span:nth-child(5){width:65%}.main-panel { display:grid; gap:12px; }.headline { display:flex; align-items:baseline; justify-content:space-between; }.headline b { font-size:17px; }.headline small { color:var(--muted); }.chart { height:137px; position:relative; overflow:hidden; border-radius:15px; background:linear-gradient(#eff2ff,#fff); }.chart:before { content:""; position:absolute; inset:13px; background:repeating-linear-gradient(to bottom,transparent 0 27px,#dfe5f2 28px 29px); }.chart svg { position:absolute; inset:12px 5px 0; width:calc(100% - 10px); height:calc(100% - 12px); }.numbers { display:grid; grid-template-columns:1fr 1fr; gap:10px; }.number { padding:11px; border:1px solid var(--line); border-radius:12px; background:#fff; }.number small { display:block; margin-bottom:4px; color:var(--muted); }.number b { font-size:15px; }.number b:before { content:""; display:inline-block; width:7px; height:7px; margin-right:6px; border-radius:50%; background:#8d9fff; }
        .receipt { position:absolute; left:-13px; bottom:4px; display:flex; align-items:center; gap:11px; padding:13px 15px; color:#fff; border-radius:15px; background:var(--navy); box-shadow:0 18px 34px #17213e38; transform:rotate(-3deg); }.receipt strong { display:block; font-size:13px; }.receipt small { display:block; margin-bottom:3px; color:#c1cbe1; font-size:10px; }.receipt-icon { width:30px; height:30px; display:grid; place-items:center; border-radius:9px; color:#17654c; background:var(--mint); font-weight:900; }
            ...
            <div class="workflow-grid">
                <article class="workflow-card focus">
                    ...
                    <svg viewBox="0 0 360 210" fill="none" aria-hidden="true">
                        ...
                        <rect x="62" y="44" width="76" height="9" rx="4.5" fill="#dce5ff" />
                ...
                <article class="workflow-card">
                    ...
                    <svg viewBox="0 0 270 190" fill="none" aria-hidden="true">
                        <rect x="39" y="29" width="174" height="121" rx="18" fill="#eef1ff" stroke="#d8dff5" />
                        <path d="m67 83 59-30 58 30-58 30-59-30Z" fill="#4058dc" fill-opacity=".18" stroke="#4058dc" stroke-width="3" />
                        <path d="m67 104 59 30 58-30M126 113v21" stroke="#4058dc" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" />
